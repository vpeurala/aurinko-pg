...
if [[ "$CONTAINER_STATUS" == "jaanmurtaja-db: Up"* ]]; then
  ...
  if [[ "$1" == "-f" || "$1" == "--force" ]]; then
    echo "You used option \"$1\", so we destroy the current container and start a new one.";
    docker rm -f jaanmurtaja-db;
    echo "Old container destroyed, starting a new one...";
  else
    echo "Run with the --force (-f) option if you want to destroy the current container and start a new one.";
